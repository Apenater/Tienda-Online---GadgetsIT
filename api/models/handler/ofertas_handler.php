...
class OfertaHandler
{
    ...
    public function searchRows()
    {
        ...
        $sql = 'SELECT id_ofertas, titulo, descripcion, descuento, idProducto, nombreProducto
        from tb_ofertas
    ...
    {
        $sql = 'SELECT id_ofertas, titulo, descripcion, descuento, nombreProducto
                FROM tb_ofertas
                INNER JOIN tb_productos USING(idProducto)
                ORDER BY titulo';
    ...
    {
        $sql = 'SELECT id_ofertas, titulo, descripcion, descuento, idProducto
                FROM tb_ofertas
                WHERE id_ofertas = ?';
    ...
    public function deleteRow()
    {
        ...
        $sqlUpdate = 'UPDATE tb_productos SET hasDiscount = 0 WHERE idProducto = (SELECT idProducto FROM tb_ofertas WHERE id_ofertas = ?)';
        $paramsUpdate = array($this->id_oferta);
        Database::executeRow($sqlUpdate, $paramsUpdate);


        // Primero, elimina la oferta en tb_ofertas
        $sqlDelete = 'DELETE FROM tb_ofertas WHERE id_ofertas = ?';
