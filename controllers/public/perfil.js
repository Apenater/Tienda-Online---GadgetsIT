...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const DATA = await fetchData(USERR_API, 'readProfile');
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
    if (DATA.status) {
        // Se inicializan los campos del formulario con los datos del usuario que ha iniciado sesión.
        const ROW = DATA.dataset;
        NOMBRE_EMPLEADO.value = ROW.nombre_cliente;
        APELLIDO_EMPLEADO.value = ROW.apellido_cliente;
        CORREO_EMPLEADO.value = ROW.correo_cliente;
        TELEFONO_EMPLEADO.value = ROW.telefono_cliente;
    } else {
...
PERFIL_FORM.addEventListener('submit', async (event) => {
    ...
    const DATA = await fetchData(USERR_API, 'editProfile', FORM);
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
    if (DATA.status) {
...
PASSWORD_FORM.addEventListener('submit', async (event) => {
    ...
    const DATA = await fetchData(USERR_API, 'changePassword', FORM);
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
    if (DATA.status) {
