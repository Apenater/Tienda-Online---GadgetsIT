...
const graficoPredictVentas = async () => {
    try {
        ...
        if (DATA.status && DATA.dataset) {
            const fechas = DATA.dataset.map(item => item.fecha);
            const ventasPrevistas = DATA.dataset.map(item => item.ventas_previstas);
            
            multiLineGraph('chart11', fechas, [
                {
                    label: 'Ventas Previstas',
                    data: ventasPrevistas
                }
            ], 'Predicción de ventas futuras');
        } else {
            console.error('Error al obtener los datos para predicción de ventas:', DATA.error || 'Sin datos');
            sweetAlert(2, DATA.error || 'No se pudieron cargar los datos de predicción de ventas.', false);
        }
    } catch (error) {
        console.error('Error al cargar la predicción de ventas:', error);
        sweetAlert(2, 'Error al cargar los datos de predicción.', false);
    }
}
